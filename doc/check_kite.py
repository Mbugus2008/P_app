...
kite = [list(r) for r in ws.iter_rows(min_row=2, values_only=True)
        if 'KITENGELA' in str(r[6] or '').upper() or 'KITENGELA' in str(r[7] or '').upper()]
...
sent_today = [r for r in kite if 'KITENGELA' in str(r[6] or '').upper() and in_range(r[2])]
recv_today = [r for r in kite if 'KITENGELA' in str(r[7] or '').upper() and in_range(r[2])]

# Paid Today: To=KITENGELA, Payment_Date=today, Date_sent!=today
paid_today = [r for r in kite
              if 'KITENGELA' in str(r[7] or '').upper()
              and in_range(r[35]) and not in_range(r[2])]

print('=== KITENGELA TODAY ===')
s_t = len(sent_today)
...
for r in paid_today[:15]:
    print(f'  {r[0]} From={r[6]} To={r[7]} DS={str(r[2])[:10]} PD={str(r[35])[:10]} Paid={r[17]} Amt={r[16]} Method={r[25]}')
